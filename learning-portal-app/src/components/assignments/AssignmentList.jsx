...
const AssignmentList = () => {
  ...
  const [
    getAssignment,
    { data: assignment = {}, isError: isErrorForgetAssignment },
  ] = useLazyGetAssignmentQuery(queryStr);

  const { data: assignments = [], totalCount } = data || {};
  ...
  useEffect(() => {
    if (assignment?.id && Number(assignment.id) === Number(assignmentId)) {
      setOpen(true);
    }

    if (assignmentId && isErrorForgetAssignment) {
      setNotFound(true);
    } else {
      setNotFound(false);
    }
  }, [assignmentId, assignment, isErrorForgetAssignment, setNotFound]);
        ...
        <table className="w-full table-auto border border-gray-600/50">
          <thead className="text-left text-slate-300 border-b border-b-gray-600/50">
            <tr>
              <th>Title</th>
              <th>Video Title</th>
              <th className="text-center">Total Mark</th>
  ...
  return notFound ? (
    <NotFound>
      <Link to="/admin/assignments" className="text-lg link mt-8">
        Go to assignmentzes Panel
      </Link>
    </NotFound>
  ) : (
    <div className="table-responsive">
      ...
      {open && (
        <Modal open={open} handleModalClose={handleModalClose}>
          <ModalFormLayout title="Edit Assignment">
            <AssignmentForm
              assignment={assignment}
